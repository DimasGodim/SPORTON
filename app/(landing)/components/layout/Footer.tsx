'use client';
import Image from 'next/image';
import Link from 'next/link'

{/* margin(jarak luar): mx(horizontal), my(vertikal), ml(left), mr(right), mt(top), mb(bottom) */}

{/*tag footer mempunyai nilai default width 100% kanan kiri tanpa perlu di atur*/}

{/*styling w-? digunakan untuk mengatur lebar content*/}
{/*styling basis-? digunakan untuk mengatur lebar main axis atau intinya lebarnya dengan responsive*/}
{/*styling container berfungsi untuk mengatur max width sesuai dengan yang ada di framework (intinya dia magic)*/}

{/*styling grid: grid-cols-? digunakan untuk mengatur batas kolom secara horizontal*/}
{/*styling grid dengan cols diurutkan item dari kiri sampai kanan batas yang diatur dan kebawah lagi dari kiri lagi sampai kanan batas yang diatur*/}

export default function Footer(){
    return (
        <footer className="bg-dark-alternate text-white mt-52">
            <div className='flex justify-between container mx-auto pt-10 pb-20'>
                
                <div className='w-105"'>
                    <Image src='/logo-footer.svg' alt="" width={127} height={30}/>
                    <p className='mt-8'>Engineered for endurance and designed for speed. <br />Experience gear that moves as fast as you do.</p>
                </div>

                <div className='w-105 grid grid-cols-2 gap-10'>
                    <div className='flex gap-5 flex-col'>
                        <Link href="">Home</Link>
                        <Link href="">Categories</Link>
                        <Link href="">Products</Link>
                        <Link href="">About Us</Link>
                    </div>
                    <div className='flex gap-5 flex-col'>
                        <Link href="">Instagram</Link>
                        <Link href="">Facebook</Link>
                        <Link href="">TikTok</Link>
                        <Link href="">YouTube</Link>
                    </div>
                </div>
            </div>
            <div className='border-t border-t-white/15'>
                <div className='container mx-auto py-6.5 flex justify-between'>
                    <div className='w-105'>SportsOn © 2025 All Rights Reserverd.</div>
                    <div className="grid grid-cols-2 w-105">
                        <Link href="">Privacy Policy</Link>
                        <Link href="">Terms Conditions</Link>
                    </div>
                </div>                
            </div>
        </footer>
    );
}